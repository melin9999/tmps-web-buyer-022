...
const ServicesBrowser = ({value, valueSelected, setOpen}) => {
  ...
  async function getServices(){
    try{
      ...
      var error = false;
      if(!error){
        const response = await axios.post("/api/services/active/", {});
        const values = [];
        ...
        {isLoading ? 
          ...
          <div className='flex flex-col justify-start items-start w-[100%] pb-2 px-3 bg-white shadow-xl' style={{width: width>=1280?1280:(width)}}>
            ...
            <div className='flex flex-row justify-start items-start w-full flex-wrap max-h-[600px] overflow-y-auto pt-2 xs:pt-5 pb-3'>
              {services.map(val=>
                <div key={val.id+val.description} className='flex flex-row justify-start items-center w-[90%] xs:w-[200px] sm:w-[200px] md:w-[280px] gap-2 py-3 px-1 mr-0 xs:mr-5 cursor-pointer hover:bg-slate-100' style={{borderRight: width>440?('1px solid #D1D5DB'):'none'}} onClick={()=>brandSelected(val)}>
